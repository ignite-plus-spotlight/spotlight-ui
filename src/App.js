        ...
        axios.get(`http://localhost:8081/${current}/employeeroles`)
          .then(res => {
            ...
            const cookies = new Cookies();
              cookies.set('roles', roles[0].roleName, { path: '/' });
              console.log(cookies.get('roles')); 
              if (roles[0].roleName == "leader")
              {
                // this.props.history.push('/Conditional')
                return ReactDOM.render(<Conditional levelType={"levelA"}/>,document.getElementById('root'))
              //  return  <DashboardV/>
              }
              else {
                ...
                {
                // this.props.history.push('/Conditional')
                  return ReactDOM.render(<Conditional levelType={"levelB"}/>,document.getElementById('root'))
                  // return <DashboardM/>
                }
                else
                {
                  if(roles[0].roleName=="team member")
                   {
                    // this.props.history.push('/Conditional')
                    return ReactDOM.render(<Conditional levelType={"levelC"}/>,document.getElementById('root'))
                    // return <DashboardT/>
                  }
